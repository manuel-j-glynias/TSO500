...
def is_in_clinvar(variant):
    if variant['in_clinvar']:
        ...
        add_lack_of_reason_to_variant(variant, 'not in ClinVar')
...
def is_clinvar_benign(variant):
    ...
    if benign_:
        add_lack_of_reason_to_variant(variant, 'marked as benign in ClinVar')
    # else:
...
def is_tumor_suppressor(variant):
    ts = variant['gene_category'] == 'Tumor Suppressor'
    if ts:
...
def not_clinvar_pathogenic(variant):
    if is_ckb_gof(variant) or is_ckb_lof(variant):
        ...
        if at_hot_spot_position(variant):
            ...
            if is_tumor_suppressor(variant):
...
def is_tso_snv_reportable(variant):
